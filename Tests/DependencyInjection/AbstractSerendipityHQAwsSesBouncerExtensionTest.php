<?php
namespace SerendipityHQ\Bundle\AwsSesBouncerBundle\Tests\DependencyInjection;

use SerendipityHQ\Bundle\AwsSesBouncerBundle\DependencyInjection\AwsSesBouncerExtension;
...
abstract class AbstractSerendipityHQAwsSesBouncerExtensionTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->extension = new AwsSesBouncerExtension();
    ...
    public function testFilterDisabledConfiguration()
    {
        ...
        $this->assertFalse($this->container->has('aws_ses_bouncer.swift_mailer.filter'));
    }
    ...
    public function testFilterEnabledConfiguration()
    {
        ...
        $this->assertTrue($this->container->has('aws_ses_bouncer.swift_mailer.filter'));
    }
    ...
    public function testFilterHasPluginTagConfiguration()
    {
        ...
        $this->assertTrue($this->container->has('aws_ses_bouncer.swift_mailer.filter'));
        $this->assertTrue($this->container->hasDefinition('aws_ses_bouncer.swift_mailer.filter'));

        $definition = $this->container->getDefinition('aws_ses_bouncer.swift_mailer.filter');
