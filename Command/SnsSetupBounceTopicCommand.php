<?php
namespace SerendipityHQ\Bundle\AwsSesBouncerBundle\Command;
...
class SnsSetupBounceTopicCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $context->setHost($container->getParameter('aws_ses_bouncer.bounce_endpoint')['host']);
        $context->setScheme($container->getParameter('aws_ses_bouncer.bounce_endpoint')['protocol']);

        $apiFactory = $container->get('aws_ses_bouncer.aws.client.factory');

        $credentials = $container->getParameter('aws_ses_bouncer.aws_config')['credentials_service_name'];
        ...
        $topicRepo = $container->get('aws_ses_bouncer.object_manager')->getRepository('AwsSesBouncerBundle:Topic');
        ...
        $subscribe = [
            'TopicArn' => $topicArn,
            'Protocol' => $container->getParameter('aws_ses_bouncer.bounce_endpoint')['protocol'],
            'Endpoint' => $this->getContainer()
                ->get('router')
                ->generate(
                    $container->getParameter(
                        'aws_ses_bouncer.bounce_endpoint'
                    )['route_name'],
                    array(),
